...
def get_data(filters):
	...
	if _from is not None and to is not None:
		...
		date_rang = f"BETWEEN '{_from}' AND '{to}'"
	else:
		frappe.throw(_("Please provide both 'from' and 'to' filters."))
	...
	return frappe.db.sql(f"""
		With pl
			as (Select tial.employee,tial.employee_name ,SUM(tial.pl_employee_contr) as total_employee_pl,SUM(tial.pl_bank_contr) as total_bank_pl,
				...
				Where tia.posting_date BETWEEN '{_from}' AND '{to}' AND tia.posting_date > em.date_of_joining
					and tia.docstatus = 1
			...
			contr as (Select tecl.employee,tecl.employee_name ,SUM(tecl.employee_contr) total_employee_contr,SUM(tecl.bank_contr) total_bank_contr,
				...
				Where tec.posting_date BETWEEN '{_from}' AND '{to}'AND tec.posting_date > em.date_of_joining
					and tec.docstatus = 1
				...
				Where tsfp.posting_date BETWEEN '{_from}' AND '{to}' AND tsfp.posting_date > em.date_of_joining
			...
			liabilty as (Select ter.employee,ter.employee_name ,ter.employee_equity_amount + ter.bank_equity_amount as liability_amount
				...
				Where ter.posting_date BETWEEN '{_from}' AND '{to}' AND ter.posting_date > em.date_of_joining
					AND ter.docstatus = 1
