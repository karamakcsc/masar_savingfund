...
class SavingFundPayment(AccountsController):
    ...
    def cancel(self):
        if len(self.get_linked_gl_entries()) > 50:
            msg = _("Saving Fund Payment cancellation is queued. It may take a few minutes")
            msg += "<br>"
            msg += _(
                "In case of any error during this background process, the system will add a comment about the error on this Saving Fund Payment and revert to the Submitted status"
            )
            frappe.msgprint(
                msg,
                indicator="blue",
